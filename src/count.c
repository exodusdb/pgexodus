...
exodus_count(PG_FUNCTION_ARGS)
{
	...
	// String to search
    text *txt_data = PG_GETARG_TEXT_PP(0);

	// Substring/char to count
    text *txt_find = PG_GETARG_TEXT_PP(1);
    ...
    // Cant count empty substring/char
    if (len_find <= 0)
        PG_RETURN_INT32(0);
	...
	// Last possible start position
    int32 max_start = len_data - len_find;

    for (int32 i = 0; i <= max_start; ++i)
    {
        if (memcmp(data + i, find, len_find) == 0)
        {
            ...
            // For overlapping (standard str.count() in Python, strpos loop), keep i++:
